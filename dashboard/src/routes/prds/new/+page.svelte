	...
	async function handleSubmit() {
		...
		try {
			...
			if (response.ok) {
				...
				error = errorData.message || 'PRD 생성 중 오류가 발생했습니다';
			}
		} catch (e) {
...
<svelte:head>
	<title>새 PRD 작성 - WorkflowMCP</title>
</svelte:head>

<div class="max-w-4xl mx-auto space-y-6">
	<div class="flex items-center justify-between">
		<div>
			<h1 class="text-3xl font-bold text-gray-900">새 PRD 작성</h1>
			<p class="text-gray-600 mt-1">프로젝트 요구사항 문서를 작성합니다</p>
		</div>
		<a href="/prds" class="btn btn-secondary">
			← 목록으로
		</a>
	</div>
	...
	<form on:submit|preventDefault={handleSubmit} class="space-y-6">
		...
		<div class="flex justify-end space-x-3">
			<a href="/prds" class="btn btn-secondary">취소</a>
			<button 
				type="submit" 
				class="btn btn-primary" 
