<script>
	...
	async function loadTemplate() {
		try {
			...
			formData = {
				template_type: template.template_type || 'snippet',
				title: template.title || '',
				description: template.description || '',
				language: template.language || '',
				framework: template.framework || '',
				category: template.category || 'snippet',
				subcategory: template.subcategory || '',
				template_code: template.code_content || '',
				usage_example: template.usage_instructions || '',
				variables: template.parameters ? JSON.stringify(template.parameters, null, 2) : '',
				tags: Array.isArray(template.keywords) ? template.keywords.join(', ') : '',
	...
	async function handleSubmit() {
		...
		try {
			...
			if (response.ok) {
				alert('코드 템플릿이 수정되었습니다.');
				goto('/code_templates/' + templateId);
			} else {
				const errorData = await response.json();
				alert('코드 템플릿 수정에 실패했습니다: ' + (errorData.error || '알 수 없는 오류'));
			}
		} catch (e) {
			alert('코드 템플릿 수정에 실패했습니다: ' + e.message);
		} finally {
			loading = false;
		}
	}
...
<svelte:head>
	<title>코드 템플릿 편집 - WorkflowMCP</title>
</svelte:head>
...
{:else}
	<div class="max-w-4xl mx-auto space-y-6">
		<div class="flex items-center justify-between">
			<div>
				<h1 class="text-3xl font-bold text-gray-900">코드 템플릿 편집</h1>
				<p class="mt-2 text-gray-600">재사용 가능한 템플릿의 세부사항을 수정합니다</p>
			</div>
			<button
				type="button"
				on:click={cancel}
